...
export const BackgroundRemoverPage: React.FC<BackgroundRemoverPageProps> = ({
  ...
  // When files change, load preview and reset orientation
  useEffect(() => {
  ...
  const handleDownload = async () => {
    ...
    setIsExporting(true);
    try {
      // Export with exact orientation (preserves 1:1 original, or user-selected flip/rotation)
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-xl mt-6">
          {!resultImageUrl ? (
            ...
            <div className="space-y-8 animate-in fade-in zoom-in-95 duration-200">
              ...
              {/* View Mode Controls, Background Switcher & Orientation Controls */}
              ...
              <div className="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-900 select-none">
                {viewMode === 'slider' && originalImageUrl && resultImageUrl ? (
                  <div className="relative w-full h-[400px] sm:h-[480px] overflow-hidden flex items-center justify-center">
                    ...
                    {/* Result image (Cutout) with exact orientation transform */}
                    <img
                      src={resultImageUrl}
                      alt="Cutout result"
                      style={{
              ...
              {/* Stats Box (Size & Orientation Info) */}
              <div className="max-w-xl mx-auto p-4 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 text-xs flex flex-wrap items-center justify-around gap-3 font-mono">
                <div>
                ...
                <div>
                  ...
                  <span className={`font-bold ${flipHorizontal ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    {flipHorizontal ? 'Mirrored (Flipped)' : '1:1 Original Preserved'}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                  <button
                    ...
                    className="w-full sm:w-auto px-8 py-4 bg-rose-600 hover:bg-rose-700 disabled:opacity-75 text-white font-bold text-sm rounded-2xl shadow-lg hover:shadow-xl shadow-rose-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >

...
export const BackgroundRemoverTool: React.FC<BackgroundRemoverToolProps> = ({ onCompleted }) => {
  ...
  useEffect(() => {
    ...
    console.log('[BackgroundRemoverTool] crossOriginIsolated:', isolated);
  }, []);
  ...
  const handleFilesSelected = (newFiles: File[]) => {
    ...
    if (!validTypes.includes(file.type.toLowerCase()) && !file.name.match(/\.(png|jpe?g|webp)$/i)) {
      setErrorMsg('Unsupported format. Please upload a PNG, JPG, or WEBP image.');
      return;
    }

    if (file.size > 25 * 1024 * 1024) {
      setErrorMsg('File size exceeds 25MB. Please choose an image under 25MB.');
      return;
    }
  ...
  // Helper test case: Red shoe on red background
  ...
  const handleExecuteRemoval = async (forceEngine?: EngineMode) => {
    if (files.length === 0) {
      setErrorMsg('Please upload an image first.');
      return;
    }

    const currentEngine = forceEngine || engineMode;
    ...
    try {
      ...
      setProgressStage(
        currentEngine === 'fallback'
          ? 'Scanning image colors for graphic cutout...'
          ...
          {files.length === 0 ? (
            <div className="space-y-4">
              <FileDropzone
                ...
                subtitle="Isolate subjects and generate high-res transparent PNGs"
              />
            ...
            <div className="space-y-6">
              ...
              {/* Original Preview */}
              {originalImageUrl && (
                <div className="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-950 max-h-[360px] flex items-center justify-center p-4">
                  <img
                    src={originalImageUrl}
                    alt="Original preview"
                    className="max-h-[320px] max-w-full object-contain rounded-lg shadow-md"
                  />
                </div>
              )}
                ...
                {engineMode === 'ai' ? (
                  <div className="space-y-4 pt-1">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      🧠 <strong>AI Neural Cutout:</strong> Executes ISNet in WebAssembly. Isolates complex foreground subjects even when foreground and background share identical colors.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4 pt-1">
                    <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-amber-800 dark:text-amber-300 text-xs">
                      ⚠️ <strong>Graphic Cutout Warning:</strong> Only use this for logos, icons, or solid screens. If your subject shares color with the background (e.g. red shoe on red background), switch to <strong>AI Neural Model</strong> above.
                    </div>
            ...
            {viewMode === 'slider' && originalImageUrl && resultImageUrl ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
                <div className="space-y-2">
                  ...
                  <div className="h-[260px] bg-slate-950 rounded-xl overflow-hidden flex items-center justify-center p-2 border border-slate-800">
                    {originalImageUrl && (
                      <img
                        src={originalImageUrl}
                        alt="Original"
                ...
                <div className="space-y-2">
                  ...
                  <div
                    className="h-[260px] rounded-xl overflow-hidden flex items-center justify-center p-2 border border-slate-800 relative"
                    style={{
                      ...getPreviewBgStyle(),
                      ...(isCheckerboard
                        ? {
